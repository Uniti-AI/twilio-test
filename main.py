...
if token and token == os.getenv('SECRET_TOKEN'):
    ...
    with n_col1:
        from_twilio_number = st.text_input('From (Lead) Number') or os.getenv('FROM_TWILIO_NUMBER')
    with n_col2:
        to_twilio_number = st.text_input('To (Agent) Number') or os.getenv('TO_TWILIO_NUMBER')
